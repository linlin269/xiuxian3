...
function renderPlayerDetail(data){const box=qs('player-detail');if(!data||!data.player){box.className='detail-box empty-state';box.textContent='请选择一位玩家。';return;}const player=data.player;const backpack=data.backpack||[];const ring=data.ring||[];const logs=data.recent_logs||[];box.className='detail-box';box.innerHTML=`<article class="info-card"><div class="title"><h4>${escapeHtml(player.display_name||'')}</h4><span class="badge">${escapeHtml(player.client_id||'')}</span></div><p>等级 ${escapeHtml(player.level ?? 0)} · 经验 ${escapeHtml(player.exp ?? 0)} · 下一级：${escapeHtml(player.next_level_text||'')}</p><div class="meta-row"><span class="badge">血气 ${escapeHtml(player.hp ?? 0)}/${escapeHtml(player.max_hp ?? 0)}</span><span class="badge">精神 ${escapeHtml(player.mp ?? 0)}/${escapeHtml(player.max_mp ?? 0)}</span><span class="badge">源石 ${escapeHtml(player.source_stones ?? 0)}</span><span class="badge">状态 ${escapeHtml(player.status||'')}</span><span class="badge">地点 ${escapeHtml(player.location_name||'')}</span></div></article><article class="info-card"><h4>库存摘要</h4><div class="meta-row"><span class="badge">背包格：${escapeHtml(data.summary?.backpack_count ?? 0)}</span><span class="badge">纳戒格：${escapeHtml(data.summary?.ring_count ?? 0)}</span><span class="badge">背包负重：${escapeHtml(data.summary?.backpack_weight ?? 0)}</span><span class="badge">源库：${escapeHtml(data.source_vault?.balance ?? 0)}</span></div></article><article class="info-card"><h4>背包</h4><div class="table-grid">${backpack.length?backpack.map((row)=>`<div class="record-card"><div class="title"><h4>${escapeHtml(row.name||row.item_id||'')}</h4><span class="badge">x${escapeHtml(row.quantity ?? 0)}</span></div><p><code>${escapeHtml(row.item_id||'')}</code> · ${escapeHtml(row.category||'')} · 品级 ${escapeHtml(row.quality||'')}</p></div>`).join(''):'<div class="empty-state">背包为空。</div>'}</div></article><article class="info-card"><h4>纳戒</h4><div class="table-grid">${ring.length?ring.map((row)=>`<div class="record-card"><div class="title"><h4>${escapeHtml(row.name||row.ring_item_id||row.item_id||'')}</h4><span class="badge">x${escapeHtml(row.quantity ?? 0)}</span></div><p><code>${escapeHtml(row.ring_item_id||row.item_id||'')}</code> · ${escapeHtml(row.category||'')} · 品级 ${escapeHtml(row.quality||'')}</p></div>`).join(''):'<div class="empty-state">纳戒为空。</div>'}</div></article><article class="info-card"><h4>最近游戏日志</h4><div class="table-grid">${logs.length?logs.map((row)=>`<div class="record-card"><div class="title"><h4>${escapeHtml(row.action||'')}</h4><span class="badge">#${escapeHtml(row.log_id ?? 0)}</span></div><p>${escapeHtml(row.detail||'')}</p><div class="meta-row"><span class="badge">${escapeHtml(row.created_at||'')}</span></div></div>`).join(''):'<div class="empty-state">暂无日志。</div>'}</div></article>`;}
