...
async def api_operations(
    ...
    _require_admin(request)
    return {"ok": True, "items": service.list_operations(limit=limit, date_from=date_from, date_to=date_to)}
...
async def api_audit_logs(
    ...
    _require_admin(request)
    return {"ok": True, "items": [_audit_log_row(row) for row in service.log_db.recent_logs(limit=limit, date_from=date_from, date_to=date_to)]}
...
def _render_dashboard(admin: dict[str, Any], status: dict[str, Any], *, active_tab: str = "overview") -> str:
    ...
    <div class="grid">
      ...
      <section id="player-detail-card" class="card panel">
        <div class="card-head"><div><h2>玩家详情</h2><p>选中某个玩家后，会在这里展示基本属性和库存摘要。</p></div></div>
      ...
      <section id="operations-card" class="card panel">
        ...
        <form id="operation-form" class="form-grid compact">
          ...
          <label class="field"><span>操作类型</span><select id="action-type" onchange="syncActionForm()"><option value="grant_stones">发放源石</option><option value="grant_item">发放物品</option><option value="grant_exp">发放经验</option></select></label>
          ...
          <label class="field exp-field hidden"><span>经验数量</span><input id="exp-amount" type="number" min="0" step="1" placeholder="例如 5000" /></label>
      ...
      <section id="records-card" class="card panel wide">
        <div class="card-head"><div><h2>操作记录</h2><p>选择类型并点击查询后显示对应记录，支持按日期范围筛选。</p></div></div>
        <div class="search-row split" style="flex-wrap:wrap;gap:8px;">
          <select id="record-type" style="min-width:160px;"><option value="operations">后台操作列表</option><option value="audit-logs">审计日志</option></select>
          <input id="date-from" type="date" placeholder="开始日期" />
          <input id="date-to" type="date" placeholder="结束日期" />
          <button type="button" class="primary" onclick="queryRecords()">查询</button>
        </div>
        <div id="record-results" class="table-list empty-state">请选择类型后点击查询。</div>
      </section>
    </div>
    """
...
function syncActionForm(){const actionType=qs('action-type').value;const stonesField=qs('stones-amount').closest('.field');const expField=qs('exp-amount').closest('.field');const itemFields=qs('item-id').closest('.item-fields');if(actionType==='grant_stones'){stonesField.classList.remove('hidden');expField.classList.add('hidden');itemFields.classList.add('hidden');}else if(actionType==='grant_exp'){stonesField.classList.add('hidden');expField.classList.remove('hidden');itemFields.classList.add('hidden');}else{stonesField.classList.add('hidden');expField.classList.add('hidden');itemFields.classList.remove('hidden');}}
async function fetchJson(url, options={}){const response=await fetch(url,{credentials:'same-origin',headers:{'Accept':'application/json','Content-Type':'application/json',...(options.headers||{})},...options});const text=await response.text();let data={};try{data=text?JSON.parse(text):{};}catch{data={ok:false,message:text||`HTTP ${response.status}`};}if(!response.ok||data.ok===false)throw new Error(data.message||data.detail||`HTTP ${response.status}`);return data;}
...
function renderPlayerDetail(data){const box=qs('player-detail');if(!data||!data.player){box.className='detail-box empty-state';box.textContent='请选择一位玩家。';return;}const player=data.player;box.className='detail-box';box.innerHTML=`<article class="info-card"><div class="title"><h4>${escapeHtml(player.display_name||'')}</h4><span class="badge">${escapeHtml(player.client_id||'')}</span></div><p>等级 ${escapeHtml(player.level ?? 0)} · 经验 ${escapeHtml(player.exp ?? 0)} · 下一级：${escapeHtml(player.next_level_text||'')}</p><div class="meta-row"><span class="badge">血气 ${escapeHtml(player.hp ?? 0)}/${escapeHtml(player.max_hp ?? 0)}</span><span class="badge">精神 ${escapeHtml(player.mp ?? 0)}/${escapeHtml(player.max_mp ?? 0)}</span><span class="badge">源石 ${escapeHtml(player.source_stones ?? 0)}</span><span class="badge">状态 ${escapeHtml(player.status||'')}</span><span class="badge">地点 ${escapeHtml(player.location_name||'')}</span></div></article><article class="info-card"><h4>库存摘要</h4><div class="meta-row"><span class="badge">背包格：${escapeHtml(data.summary?.backpack_count ?? 0)}</span><span class="badge">纳戒格：${escapeHtml(data.summary?.ring_count ?? 0)}</span><span class="badge">背包负重：${escapeHtml(data.summary?.backpack_weight ?? 0)}</span><span class="badge">源库：${escapeHtml(data.source_vault?.balance ?? 0)}</span></div></article>`;}
function renderOperations(items){const box=qs('record-results');if(!items||!items.length){box.className='table-list empty-state';box.textContent='暂无后台操作记录。';return;}box.className='table-grid';box.innerHTML=`<div class="table-head"><div>ID</div><div>状态</div><div>目标</div><div>物品/源石/经验</div><div>时间</div><div>操作</div></div>${items.map((row)=>`<div class="table-row"><div><code>#${escapeHtml(row.operation_id ?? 0)}</code></div><div><span class="badge ${row.status==='success'?'success':row.status==='failed'?'warning':''}">${escapeHtml(row.status||'')}</span></div><div><div>${escapeHtml(row.target_name_snapshot||row.target_client_id||'')}</div><div class="mini">${escapeHtml(row.target_client_id||'')}</div></div><div><div>${row.action_type==='grant_stones'?`源石 ${escapeHtml(row.stones_amount ?? 0)}`:row.action_type==='grant_exp'?`经验 ${escapeHtml(row.quantity ?? 0)}`:`${escapeHtml(row.item_name_snapshot||row.item_id_snapshot||'')} x${escapeHtml(row.quantity ?? 0)}`}</div><div class="mini">${escapeHtml(row.reason||'')}</div></div><div class="mini">${escapeHtml(row.created_at||'')}</div><div><button type="button" class="link-button ghost" onclick="viewOperation(${Number(row.operation_id||0)})">查看</button></div></div>`).join('')}`;}
function renderAuditLogs(items){const box=qs('record-results');if(!items||!items.length){box.className='table-list empty-state';box.textContent='暂无审计日志。';return;}box.className='table-grid';box.innerHTML=`<div class="table-head"><div>日志</div><div>阶段</div><div>结果</div><div>目标/物品</div><div>原因</div><div>时间</div></div>${items.map((row)=>`<div class="table-row"><div><code>#${escapeHtml(row.log_id ?? 0)}</code></div><div><span class="badge">${escapeHtml(row.stage||'')}</span></div><div><span class="badge ${row.status==='success'?'success':row.status==='failed'?'warning':''}">${escapeHtml(row.status||'')}</span></div><div><div>${escapeHtml(row.target_name_snapshot||row.target_client_id||'')}</div><div class="mini">${escapeHtml(row.item_name_snapshot||row.item_id_snapshot||'')}</div></div><div class="mini">${escapeHtml(row.error_message||'')}</div><div class="mini">${escapeHtml(row.created_at||'')}</div></div>`).join('')}`;}
function renderPreview(operation){const box=qs('preview-panel');if(!operation){box.className='preview-box empty-state';box.textContent='还没有生成预览。';return;}currentPreview=operation;box.className='preview-box';const typeLabel=operation.action_type==='grant_stones'?'源石':operation.action_type==='grant_exp'?'经验':'物品';const actionLabel=operation.action_type==='grant_stones'?'源石':operation.action_type==='grant_exp'?'经验':operation.item_name_snapshot||operation.item_id_snapshot||'';box.innerHTML=`<article class="preview-card"><div class="title"><h4>预览 #${escapeHtml(operation.operation_id ?? 0)}</h4><span class="badge">${escapeHtml(operation.status||'')}</span></div><div class="meta-row"><span class="badge">请求编号 ${escapeHtml(operation.request_id||'')}</span><span class="badge">管理员 ${escapeHtml(operation.admin_username||'')}</span><span class="badge">目标 ${escapeHtml(operation.target_name_snapshot||operation.target_client_id||'')}</span><span class="badge">${escapeHtml(typeLabel===actionLabel?typeLabel:typeLabel+' '+actionLabel)}</span></div><p>原因：${escapeHtml(operation.reason||'')} · 备注：${escapeHtml(operation.note||'')}</p><div class="preview-json"><div><h5>执行前</h5><pre>${escapeHtml(JSON.stringify(operation.before||{}, null, 2))}</pre></div><div><h5>执行后</h5><pre>${escapeHtml(JSON.stringify(operation.after||{}, null, 2))}</pre></div></div><div><h5>警告</h5>${(operation.warnings||[]).length?`<ul>${(operation.warnings||[]).map((item)=>`<li>${escapeHtml(typeof item==='string'?item:JSON.stringify(item))}</li>`).join('')}</ul>`:'<div class="empty-state">没有警告。</div>'}</div></article>`;}
